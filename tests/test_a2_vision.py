...
from backend.agents.a2_vision import NON_BAG_ATTEMPT_LIMIT, A2VisionAgent
...
async def test_a2_skips_with_no_images():
    agent = make_agent()
    agent._vision.analyze_image = AsyncMock()
...
async def test_a2_skips_when_only_tag_photos():
    ...
    result = await agent.handle(
        make_state(image_paths=["uploads/claim_01/tag_front.jpg"]), []
    )
    assert result.damage_types == [] and result.severity_score == 0.0
    agent._vision.analyze_image.assert_not_called()
...
async def test_a2_analyzes_single_damage_image():
    agent = make_agent()
    agent._vision.analyze_image = AsyncMock(
        return_value=scene(damage_types=["cracked wheel"], severity_score=0.5)
    )
    result = await agent.handle(
        make_state(image_paths=["uploads/claim_01/damage_front.jpg"]), []
    )
    ...
    result = await agent.handle(
        make_state(
            image_paths=[
                ...
                "uploads/claim_01/tag_barcode.jpg",
            ]
        ),
        [],
    )
...
async def test_a2_sets_is_luxury_for_luxury_brand():
    ...
    result = await agent.handle(
        make_state(image_paths=["uploads/claim_01/rimowa_damage.jpg"]), []
    )
...
async def test_a2_sets_re_request_on_low_confidence():
    agent = make_agent()
    agent._vision.analyze_image = AsyncMock(
        return_value=scene(severity_score=0.5, damage_confidence=0.2)
    )
    result = await agent.handle(
        make_state(image_paths=["uploads/claim_01/blurry.jpg"]), []
    )
...
async def test_a2_sets_error_on_provider_failure():
    ...
    result = await agent.handle(
        make_state(image_paths=["uploads/claim_01/damage.jpg"]), []
    )
...
async def test_a2_flags_non_bag_image():
    ...
    result = await agent.handle(
        make_state(image_paths=["uploads/claim_01/damage_watch.jpg"]), []
    )
...
async def test_a2_non_bag_then_real_bag_resets_counter():
    agent = make_agent()
    agent._vision.analyze_image = AsyncMock(
        return_value=scene(damage_types=["cracked shell"], severity_score=0.6)
    )
    result = await agent.handle(
        make_state(image_paths=["uploads/claim_01/damage_bag.jpg"], non_bag_attempts=2),
        [],
    )
...
async def test_a2_detects_tag_in_damage_photo():
    agent = make_agent()
    agent._vision.analyze_image = AsyncMock(
        return_value=scene(severity_score=0.5, tag_visible=True, tag_confidence=0.85)
    )
    result = await agent.handle(
        make_state(image_paths=["uploads/claim_01/damage_with_label.jpg"]), []
    )
...
async def test_a2_ignores_low_confidence_tag_in_photo():
    agent = make_agent()
    agent._vision.analyze_image = AsyncMock(
        return_value=scene(severity_score=0.5, tag_visible=True, tag_confidence=0.4)
    )
    result = await agent.handle(
        make_state(image_paths=["uploads/claim_01/damage_blurry_label.jpg"]), []
    )
