...
class A1ConversationAgent(BaseAgent):
    ...
    etc. populated when A1 builds its LLM messages. A1 injects this analysis
    context into the prompt so the passenger-facing reply includes a clear
    ...
    def _build_analysis_context(self, state: ClaimState) -> str:
        """
        Build a context block summarising A2/A3 findings for injection into A1's prompt.
        Only includes fields that are actually populated. Returns empty string if nothing
        has been analysed yet (text-only turn before any images uploaded).

        This context is appended as a hidden [ANALYSIS] block in the message list so
        the LLM can reference it when composing the passenger reply.
        """
        parts = []

        # A2 damage analysis results
        ...
        # A4 routing result (if already decided)
    ...
    def _get_step_prompt(self, state: ClaimState, prompts: dict) -> str:
        ...
        Priority:
        1. re_request_tag / re_request_damage — retry prompts
        2. result step (routing_lane set by A4)
        3. *_received variants — photos just uploaded for this step
        4. Normal step prompt
        """
        ...
        if state.conversation_step == "result":
            if state.routing_lane == 1:
                ...
                return steps.get("confirm", "")

        # Image-received acknowledgment: use *_received variant when
        # photos are present for the current step
        ...
        if state.conversation_step == "damage_photos" and has_damage_images:
            return steps.get("damage_photos_received", steps.get("tag_photo", ""))

        if state.conversation_step == "tag_photo" and has_tag_images:
    ...
    def _build_messages(
        ...
        2. Last 6 turns of conversation history
        3. Passenger's current message
        4. Analysis context from A2/A3 (if any)
        5. Step instruction (hidden from passenger)
        """
        ...
        # Inject A2/A3 analysis results so A1 can reference them
    ...
    def _advance_step(self, state: ClaimState) -> str:
        ...
        Never advances if re_request flags are set or routing_lane is already
        decided. Jumps to 'result' as soon as routing_lane is set by A4.
        """
